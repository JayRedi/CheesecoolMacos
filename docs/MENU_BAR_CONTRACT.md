...
真实指标的格式为 `42°C`、`18%` 和 `6.8 W`，不会应用自定义字体。风扇 RPM 和占空比在后续 HID 阶段前仍代表 `FakeHostDevice`/未连接的开发数据，不会呈现为真实硬件遥测数据。

每个状态栏项目都提供同一个精简的原生上下文菜单，仅包含“设置”和“退出”。运行模式和指标可见性在“设置”窗口中配置。每个可见的 CheeseCool 入口始终可以打开“设置”。
